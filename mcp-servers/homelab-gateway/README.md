...
One container. 23 tool modules. 150+ sub-tools. Two MCP tools per module.
...
This matters because LLMs have limited context windows. Registering 150+ tools individually would consume most of the available context just listing tool definitions. With the `_list`/`_call` pattern, the LLM sees only 46 tools (2 per module), and can discover sub-tools on demand.
...
| `projects` | `projects_list`, `projects_call` | 23 | Project pipeline CRUD (projects, research, sprints, artifacts) |
